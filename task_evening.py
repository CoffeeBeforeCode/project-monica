# task_evening.py
# Why: Creates the evening pill reminder at 17:00 London local time daily.
# Kept separate from the morning trigger so a problem with either does not
# affect the other.
...
def today_london_at(hour: int, minute: int = 0) -> datetime:
    ...
    17:00 London always means 17:00 on your clock — 16:00 UTC in BST,
    17:00 UTC in GMT. No manual adjustment needed at clock changes.
    """
...
def createEveningTasks(timer: func.TimerRequest) -> None:
    """
    Why: Fires at 17:00 London local time daily. The evening pill task is
    created at this point so it never appears in the morning list — it only
    becomes relevant from early evening onwards.
    """
    logging.info("createEveningTasks fired")
    token = get_access_token()
    if not token:
        return
    create_todo_task(token, HOME_LIST_ID, "Take: Evening pill", "[01] Self", due_utc=today_london_at(17, 0))
