# task_evening.py
# Why: Creates the evening pill reminder at 17:00 UTC daily. Kept separate
# from the morning trigger so a problem with either does not affect the other.
...
def createEveningTasks(timer: func.TimerRequest) -> None:
    """
    Why: Fires at 17:00 UTC daily. The evening pill task is created at this
    point so it never appears in the morning list — it only becomes relevant
    ...
    if not token:
        return
    create_todo_task(token, HOME_LIST_ID, "Take: Evening pill", "[01] Self", due_utc=today_utc_at(17, 0))
