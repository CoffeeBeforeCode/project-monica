...
def createThursdayEveningTasks(timer: func.TimerRequest) -> None:
    """
    Why: Fires every Thursday at 17:00 London local time.
    A separate evening trigger is required because createThursdayTasks
    ...
    """
    logging.info("createThursdayEveningTasks fired")
    token = get_access_token()
    if not token:
        return

    create_todo_task(token, HOME_LIST_ID, "Change: Tea Towel", "[00] System", due_utc=today_london_at(17, 0))
