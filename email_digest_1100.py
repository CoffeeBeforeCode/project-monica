"""
email_digest_1100.py — Monica Email Digest  |  11:00 slot

Fires daily at 11:00 London local time, seven days a week.
...
Slot behaviour:
  - Concertina email triage card with Leo's 11:00 greeting in the header.
...
def emailDigest1100(timer: func.TimerRequest) -> None:
    ...
    if emails:
        ...
        logging.info(f"emailDigest1100: concertina card delivered — {len(emails)} email(s)")
    else:
        _send_card_to_teams(
            _build_clear_inbox_card(greeting=slot_greeting, count_line=count_line)
        )
        logging.info("emailDigest1100: clear inbox card delivered")
