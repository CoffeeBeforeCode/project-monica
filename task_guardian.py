...
#   The morning task triggers fire at 05:00 London local time. If the host is
#   not fully stable at that moment — for example, immediately after a restart
#   or an infrastructure event — the triggers can be silently lost. Azure does
#   not replay missed timer events.
#
#   This guardian fires at 05:15 London local time every day — fifteen minutes
#   after the task triggers. It checks whether every expected task for today
#   exists in To Do. If any are missing, it creates them and sends a Leo alert
#   to the Daily Operations channel so the failure is never silent.
#
...
#
# Session 31 fix:
#   today_utc_at replaced with today_london_at throughout. Due times and
#   weekday calculations now use London local time so tasks display at the
#   correct local time year-round regardless of GMT/BST.
...
def today_london_at(hour: int, minute: int = 0) -> datetime:
    ...
    05:00 UTC in GMT. Using London time here ensures recovered tasks have
    the same due times as tasks created by the original triggers.
    """
...
def task_exists_today(token: str, list_id: str, title: str) -> bool:
    """
    Why: Checks whether a task with the given title was created today (London
    date) in the specified list. Uses London date rather than UTC date so the
    check is correct near midnight when the UTC and London dates may differ.
    Returns True if found — no recovery needed.
    Returns False if not found OR if the query fails — safer to recreate
    than to skip recovery on a query error.
    """
        ...
        for task in tasks:
            if task.get("title", "").strip() == title.strip():
                ...
                if created_raw:
                    ...
                    ).astimezone(LONDON_TZ).date()
                    if created_date == today_london:
                        return True
        return False
    except Exception as e:
...
def send_teams_alert(recovered: list[str]) -> None:
    ...
    try:
        ...
        if resp.status_code in (200, 201):
            ...
            logging.error(f"Guardian: Teams alert failed — {resp.status_code} {resp.text}")
    except Exception as e:
...
def get_expected_tasks_today() -> list[dict]:
    """
    Why: The single source of truth for what tasks should exist on any given
    day. Mirrors the logic in the individual task files exactly — same titles,
    same list IDs, same categories, same alternating-week calculation for Friday.
    Why weekday uses London time:
      Near midnight UTC the UTC and London dates can differ by a day. Using
      London weekday ensures the guardian checks for the correct day's tasks.
    Sunday tasks (Wash: Napkins) fire at 17:00 London time, after this
    guardian's 05:15 window. Excluded to avoid false recoveries every Sunday.
    Evening tasks (Take: Evening pill) fire at 17:00 London time for the same
    reason and are excluded on the same logic.
    """
    now_utc    = datetime.now(timezone.utc)
    now_london = now_utc.astimezone(LONDON_TZ)
    weekday    = now_london.weekday()  # Monday=0 … Sunday=6
    ...
    if weekday == 0:  # Monday
        tasks += [
            ...
            },
            {
                ...
                "category":     "[00] System",
                "due_utc":      morning,
                "reminder_utc": today_london_at(9, 0),
            },
        ]
        ...
        weeks_since_bath  = (now_utc - bath_towels_start).days // 7
        ...
        weeks_since_bedding = (now_utc - bedding_start).days // 7
...
@bp.timer_trigger(
    schedule="0 15 5 * * *",   # 05:15 London local time every day
    arg_name="timer",
...
def taskGuardian(timer: func.TimerRequest) -> None:
    """
    Why: Fires every day at 05:15 London local time — fifteen minutes after
    the morning and day-of-week triggers at 05:00. Checks every expected task
    for today. Recovers any that are missing. Alerts Leo's Daily Operations
    channel if recovery was needed. Logs a clean OK if everything was present.
    ...
    if recovered:
        logging.warning(f"taskGuardian: Recovered {len(recovered)} missing task(s): {recovered}")
