...
def today_london_at(hour: int, minute: int = 0) -> datetime:
    ...
    The old today_utc_at() used raw UTC, which is correct in winter but
    one hour behind during BST — replaced here for consistency.
    """
...
def createWednesdayTasks(timer: func.TimerRequest) -> None:
    """
    Why: Fires every Wednesday at 05:00 London local time. Black Wednesday
    laundry and the weekly vacuum. The vacuum gets a 09:00 London reminder
    so it surfaces again mid-morning if not yet done.
    ...
    create_todo_task(token, HOME_LIST_ID,  "Wash: Black Wednesday",    "[00] System", due_utc=morning)
