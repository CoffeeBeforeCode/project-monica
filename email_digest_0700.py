"""
email_digest_0700.py — Monica Email Digest  |  07:00 slot

Fires daily at 07:00 London local time, seven days a week.
...
Slot behaviour:
  - Concertina email triage card with Leo's 07:00 greeting in the header.
  - Clear inbox card if inbox is empty.
  - No weather, agenda, or market data — those belong to the 05:00 briefing.
"""
...
def emailDigest0700(timer: func.TimerRequest) -> None:
    ...
    if emails:
        ...
        logging.info(f"emailDigest0700: concertina card delivered — {len(emails)} email(s)")
    else:
        _send_card_to_teams(
            _build_clear_inbox_card(greeting=slot_greeting, count_line=count_line)
        )
        logging.info("emailDigest0700: clear inbox card delivered")
