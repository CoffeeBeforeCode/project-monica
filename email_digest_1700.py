"""
email_digest_1700.py — Monica Email Digest  |  17:00 slot

Fires daily at 17:00 London local time, seven days a week.
...
Slot behaviour:
  - Concertina email triage card with Leo's 17:00 greeting in the header.
...
def emailDigest1700(timer: func.TimerRequest) -> None:
    ...
    if emails:
        ...
        logging.info(f"emailDigest1700: concertina card delivered — {len(emails)} email(s)")
    else:
        _send_card_to_teams(
            _build_clear_inbox_card(greeting=slot_greeting, count_line=count_line)
        )
        logging.info("emailDigest1700: clear inbox card delivered")
