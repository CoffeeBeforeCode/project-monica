"""
email_digest_0900.py — Monica Email Digest  |  9:00 slot

Fires daily at 9:00 London local time, seven days a week.
...
Slot behaviour:
  - Concertina email triage card with Leo's 9:00 greeting in the header.
...
def emailDigest0900(timer: func.TimerRequest) -> None:
    ...
    if emails:
        ...
        logging.info(f"emailDigest0900: concertina card delivered — {len(emails)} email(s)")
    else:
        _send_card_to_teams(
            _build_clear_inbox_card(greeting=slot_greeting, count_line=count_line)
        )
        logging.info("emailDigest0900: clear inbox card delivered")
